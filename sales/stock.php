...
$file = $_FILES['stock_file']['name'];
$tmp_name = $_FILES['stock_file']['tmp_name'];
...
if(strcasecmp(trim($first_val[0],"*"),"STORE_CODE")!=0 || strcasecmp(trim($first_val[1],"*"),"EAN")!=0 || strcasecmp(trim($first_val[2],"*"),"STOCK_DATE")!=0 || strcasecmp(trim($first_val[3],"*"),"QTY")!=0 || strcasecmp(trim($first_val[4],"*"),"Chain Name")!=0 || strcasecmp(trim($first_val[5],"*"),"Store Name")!=0)
{
...
echo '</table>';
*/
  $get_last_id = mysql_query("SELECT max(id) FROM stock");
...
for ($col = 0; $col < $highestColumnIndex; ++ $col) {
  ...
  if(is_null($val[0])||is_null($val[1])||is_null($val[2])||is_null($val[3])||is_null($val[5]))
  {
  ...
  { 
    header("location:index.php?msg=e5");
    if($last_id=="")
      $sql_del = "DELETE FROM $dropdownval";
    else  
      $sql_del = "DELETE FROM $dropdownval WHERE id > $last_id";
  ...
  $sql="INSERT INTO $dropdownval(`store_code`,`ean`,`stock_date`,`soh`,`chain_name`,`store_name`,`distributor_name`,`uploader_id`)
  VALUES('".$val[0]."','".$val[1]."','".$val[2]."','" .$val[3]."','".$val[4]."','".$val[5]."','".$get_dist_name_r."','".$user_id."')";
